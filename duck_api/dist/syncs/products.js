...
const syncProducts = async (db, token) => {
    ...
    `);
    let terminalsResponse = await fetch(`https://les-ailes-co-co.iiko.it/resto/api/v2/entities/products/list?includeDeleted=true&key=${token}&types=DISH&types=MODIFIER`, {
        method: "GET",
        headers: {
    ...
    const sqlQuery = `
     ${data.map((item) => `INSERT INTO products 
