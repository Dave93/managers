...
export class IikoDictionariesService {
  ...
  async getBalanceStores(token: string) {
    ...
    for (const store of storeId) {
      ...
      const existingBalanceStores = await drizzleDb
        ...
        try{
          await drizzleDb
          .delete(balance_store)
          .where(eq(balance_store.enddate, date))
          .execute()
        }catch(e){
          console.log("error deleting balance_store", e)
          process.exit(1)
        }
      ...
      for (const balanceStore of storeResponce) {
        ...
        try{
          ...
          console.log("error inserting balance_store", e)
          process.exit(1)
        }
  ...
  async getCorporationDepartments(token: string) {
    ...
    const corporation_departments = await drizzleDb
      ...
      const parsedResult = await xml2js.parseStringPromise(corporation);
      // console.log("result", result);
      for (const corporation_departmentTable of parsedResult.corporateItemDtoes
        .corporateItemDto) {
  ...
  async getCorporationGroups(token: string) {
    ...
    const existingCorporationGroups = await drizzleDb
      ...
      const parsedResult = await xml2js.parseStringPromise(corporationGroups);
      // console.log("parsedResult", parsedResult);
      for (const corporation_groupTable of parsedResult.groupDtoes.groupDto) {
  ...
  async getReportOlap(token: string) {
    ...
    const response = await this.fetchWithRetry(
      `https://les-ailes-co-co.iiko.it/resto/api/v2/reports/olap?key=${token}`,
      {
        ...
        body: JSON.stringify({
          ...
          aggregateFields: ["Amount.Out", "DishAmountInt.Amount"],
          filters: {
            "DateTime.DateTyped": {
    ...
    for (const reportOlaps of reportOlap.data) {
      ...
      const amount = reportOlaps["Product.Type"] === "DISH"
        ? reportOlaps["DishAmountInt.Amount"]
  ...
  async getCorporatinStore(token: string) {
    try {
      ...
      const corpStoreTable = await drizzleDb
        ...
        for (const corpStoreTables of parsedResult.corporateItemDtoes
          .corporateItemDto) {
          let storeItem = {
            id:
  ...
  async getOutgoingInvoice(token: string, type: string = "outgoing") {
    ...
    let incomeDoc = [];
    const incomeDocs = await drizzleDb
    .select()
    .from(invoices)
    .where(
      and(
        eq(invoices.type, "outgoing"),
        gte(invoices.incomingDate, dayjs(fromDate).subtract(20, "day").toISOString()),
        lte(invoices.incomingDate, dayjs(toDate).add(1, "day").toISOString())
      )
    )
    .execute();
    
    const result = await xml2js.parseStringPromise(receiptInvoice);
      ...
      for (const record of docs) {
        ...
        if (!existingIncomeDoc) {
          ...
          if (record.items && record.items[0] && record.items[0].item) {
  ...
  async getIncomingInvoice(token: string, type: string = "incoming") {
    ...
    console.log('incomeDocs', incomeDocs.length)


    let result = await xml2js.parseStringPromise(receiptInvoice);
    // console.log("result", result);
  ...
  async getSupplers(token: string) {
    ...
    const parsedResult = await xml2js.parseStringPromise(supplierText);

    for (const supplier of parsedResult.employees.employee) {
...
const iikoService = new IikoDictionariesService(client);
async function runIikoSync() {
  console.log('before start ' + dayjs().format('YYYY-MM-DD HH:mm:ss'))
  await iikoService.getIikoDictionariesFromIiko();
