...
const fetchPopularDishesData = async (
  ...
  const { data, error, status } = await apiClient.api.charts["popular-dishes"].get({
    query
  });
...
const PopularDishesChart = () => {
  ...
  const formatCompactNumber = (number: number) => {
    ...
    if (Math.abs(number) >= billion) {
      ...
      return `${(number / million).toFixed(0)} млн`;
    } else if (Math.abs(number) >= thousand) {
      return `${(number / thousand).toFixed(0)} тыс`;
    }
        ...
        <CardContent className="p-2 grow flex flex-col">
          <ResponsiveContainer width="100%" height="100%">
            ...
            >
              ...
              <YAxis dataKey="name" type="category" width={150} minTickGap={2} tickFormatter={(value) => {
                return value.length > 12 ? value.substring(0, 12) + '...' : value;
              }} />
              <Tooltip labelStyle={{ color: 'black' }} contentStyle={{ backgroundColor: 'white' }} />
              <Legend />
              <Bar dataKey="value" fill="#8884d8" name={t('charts.PopularDishesChart.countOfSales')} />
            </BarChart>
          </ResponsiveContainer>
