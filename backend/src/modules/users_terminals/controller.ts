...
export const usersTerminalsController = new Elysia({
  ...
  .get(
    ...
    {
      permission: "users_terminals.list",
      query: t.Object({
        limit: t.String(),
        offset: t.String(),
  ...
  .post(
    "/users_terminals/assign_terminals",
    async ({ body: { data }, user, set, drizzle, cacheController }) => {
      await drizzle
        .delete(users_terminals)
        .where(eq(users_terminals.user_id, data.user_id))
        .execute();

      const users_terminalsList = await drizzle
        .insert(users_terminals)
        .values(
          data.terminal_id.map((terminal_id: string) => ({
    ...
    {
      permission: "users_terminals.edit",
      body: t.Object({
        data: t.Object({
          user_id: t.String(),
