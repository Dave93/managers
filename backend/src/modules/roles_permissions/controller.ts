...
export const rolesPermissionsController = new Elysia({
    ...
    .use(ctx)
    .get('/roles_permissions', async ({ query: { limit, offset, sort, filters, fields }, user, set, drizzle }) => {
        if (!user) {
            set.status = 401;
            return {
    ...
    .post('/roles_permissions', async ({ body: { data }, user, set, drizzle, cacheController }) => {
        if (!user) {
            set.status = 401;
            return {
                message: 'User not found'
