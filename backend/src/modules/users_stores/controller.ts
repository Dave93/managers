...
export const usersSroresController = new Elysia({
  ...
  .get("/users_stores/cached", async ({ user, set, drizzle, cacheController }) => {
    if (!user) {
      set.status = 401;
      return {
        message: "User not found",
    ...
    const usersStoresList = await drizzle.select({
    ...
    if (usersStoresList.length > 0) {
      const storeIds = usersStoresList.map((store: any) => store.corporation_store_id);
