  ...
  .post(
    ...
    async ({
      ...
      const organizationCredentials =
        await cacheController.getCachedCredentials({});

      const credentials = organizationCredentials.filter(
        (item: any) =>
          item.model_id === organization.id && item.model == "organization"
      );

      const iikoId = credentials.find(
        (item: any) => item.type === "iiko_id"
      )?.key;

      const iikoLogin = credentials.find(
        (item: any) => item.type === "iiko_login"
      )?.key;
      ...
      response = await fetch(`https://api-ru.iiko.services/api/1/stop_lists`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
