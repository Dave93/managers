...
export const reportsController = new Elysia({
    ...
    .use(ctx)
    .get('/reports', async ({ query: { limit, offset, sort, filters, fields }, user, set, drizzle }) => {
        if (!user) {
            set.status = 401;
            return {
    ...
    .get('/reports/my_reports', async ({ query: { sort, filters, fields, terminal_id }, user, set, drizzle }) => {
        if (!user) {
            set.status = 401;
            return {
                message: 'User not found'
    ...
    .get('/reports/:id', async ({ params: { id }, user, set, drizzle }) => {
        if (!user) {
            set.status = 401;
            return {
                message: 'User not found'
    ...
    .post('/reports/by_date', async ({ body: { date, terminal_id, time }, user, set, drizzle, cacheController }) => {
        if (!user) {
            set.status = 401;
            return {
                message: 'User not found'
        ...
        const reportsList = await drizzle.select().from(reports).where(and(
            eq(reports.date, dayjs.unix(+date).hour(0).minute(0).second(0).toISOString()),
            eq(reports.terminal_id, terminal_id)
        )).execute();
        ...
        if (reportsList.length == 0) {
            ...
            if (clickReportResult.status == "rejected") {
                ...
                result.incomes.push({
                    error: null,
                    amount: clickReportResult.value.data && typeof clickReportResult.value.data == 'number' ? clickReportResult.value.data : 0,
                    type: "click",
                    readonly: true,
                    label: "Click",
                });
            ...
            if (paymeReportResult.status == "rejected") {
                ...
                result.incomes.push({
                    error: null,
                    amount: paymeReportResult.value.data && typeof paymeReportResult.value.data == 'number' ? paymeReportResult.value.data : 0,
                    type: "payme",
                    readonly: true,
                    label: "Payme",
                });
            ...
            if (yandexReportResult.status == "rejected") {
                ...
                result.incomes.push({
                    error: null,
                    amount: yandexReportResult.value.data && typeof yandexReportResult.value.data == 'number' ? yandexReportResult.value.data : 0,
                    type: "yandex",
                    readonly: true,
                    label: "Yandex",
                });
            ...
            if (arrytReportResult.status == "rejected") {
                ...
                if (arrytReportResult.value.data && 'withdraws' in arrytReportResult.value.data) {
                    arrytReportResult.value.data.withdraws.forEach((item) => {
            ...
            if (iikoCachierReport.status == "fulfilled" && iikoCachierReport.value.data && 'totalSum' in iikoCachierReport.value.data) {
            ...
            if (editableStatusIds.includes(reportItem.status_id)) {
                const existingReportItems = await drizzle
                    ...
                    .from(reports_items)
                    .where(and(
                        inArray(reports_items.source, ['cash', 'uzcard', 'humo', 'uzum_tezkor', 'other_expenses']),
                ...
                const reportItems = await drizzle.select().from(reports_items).where(eq(reports_items.report_id, reportItem.id)).execute();
    ...
    .post('/reports', async ({ body: {
        ...
        incomes,
        expenses
    }, user, set, drizzle, cacheController }) => {
        if (!user) {
            set.status = 401;
            return {
                message: 'User not found'
        ...
        if (reportsList.length > 0) {
            ...
            if (reportStatus && ['checking', 'confirmed'].includes(reportStatus.code)) {
                set.status = 400;
                return {
        ...
        if (reportsList.length == 0) {
            ...
            let difference = 0;
            if (iikoCachierReport.value.data && 'totalSum' in iikoCachierReport.value.data && iikoCachierReport.value.data.totalSum) {
                difference = totalManagerPrice - iikoCachierReport.value.data.totalSum;
            }
            ...
            let resReports = await drizzle.insert(reports).values({
                date: typeof date == 'number' ? dayjs.unix(+date).hour(0).minute(0).second(0).toISOString() : dayjs(date).hour(0).minute(0).second(0).toISOString(),
                terminal_id: terminal_id,
                status_id: sentReportStatus?.id ?? "",
                user_id: user.user.id,
                cash_ids: iikoCachierReport.value.data && 'cashIds' in iikoCachierReport.value.data ? iikoCachierReport.value.data.cashIds : [],
                total_amount: iikoCachierReport.value.data && 'totalSum' in iikoCachierReport.value.data ? iikoCachierReport.value.data.totalSum : 0,
            ...
            const reportItem = reportsList[0];
            await drizzle.delete(reports_items).where(eq(reports_items.report_id, reportItem.id)).execute();
            ...
            await drizzle.update(reports).set({
                ...
                difference,
                arryt_income: arrytIncome,
            }).where(eq(reports.id, reportItem.id)).execute();
            const reportItemsData = reportItems.map((item) => ({
                ...item,
