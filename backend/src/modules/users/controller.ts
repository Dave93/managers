...
import { users, users_roles, users_terminals } from "@backend/../drizzle/schema";
...
export const usersController = new Elysia({
    ...
    .post("/users/assign_terminal", async ({ body: { user_id, terminal_id }, user, set, drizzle }) => {
        if (!user) {
            set.status = 401;
            return {
                message: "User not found",
        ...
        await drizzle
            ...
            .execute();
        await drizzle.insert(users_terminals).values(terminal_id.map(item => ({ user_id, terminal_id: item }))).execute();
        return {
            data: {
                user_id,
                terminal_id,
